...
start(Parent, Table_id, Socket, Peer_id) ->
    spawn(?MODULE, recv_loop, [Parent, Table_id, Socket, Peer_id]).

recv_loop(Parent, Table_id, Socket, Peer_id) ->    
	...
	{ok, _Data} ->
	    ...
	    recv_loop(Parent, Table_id, Socket, Peer_id); %% not sure
	{error, Reason} ->
