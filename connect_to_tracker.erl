...
init(Dl_pid, Peers_pid, Length, File_storage_pid) ->
    ...
    loop(Info_sha, 10000, My_id, none, "6881", Length, Peers_pid, Dl_pid, File_storage_pid).
loop(Info, Time, My_id, Tracker, Port,Length, Peers_pid, Dl_pid, File_storage_pid) ->
    receive
	{connect, From, H} ->
	    ...
	    From ! {ok, Peers},
	    loop(Info, Min_time, My_id, H, Port, Length, Peers_pid, Dl_pid, File_storage_pid)
    after 10000 ->
	    case Tracker of
		none ->
		    loop(Info, Time, My_id, Tracker, Port, Length, Peers_pid, Dl_pid, File_storage_pid);
