...
create_window() ->
    ...
    Frame = wxFrame:new(Wx, 
			...
			[{size, {600, 180}}, {style, 
?wxDEFAULT_FRAME_STYLE band bnot ?wxRESIZE_BORDER band bnot ?wxMAXIMIZE_BOX}]),
    ...
    wxToolBar:addTool(ToolBar, ?wxID_OPEN, "Open",  Open, 
		      [{shortHelp, "Open"}]),
    wxToolBar:setToolLongHelp(ToolBar, ?wxID_OPEN, "Open's a torrent"),
    wxToolBar:insertSeparator(ToolBar, 2),  
    wxToolBar:addCheckTool(ToolBar, ?ADVANCE_FUNCTIONS_END, "Advance", 
			   Advance_Start, [{shortHelp, "Advance"}, 
				     {longHelp, "Activate Advance Function"}]),
 
    wxToolBar:insertSeparator(ToolBar, 4),
    wxToolBar:addTool(ToolBar, ?wxID_HELP, "Help", Help , 
		      [{shortHelp, "Help"}]),   
    wxToolBar:setToolLongHelp(ToolBar, ?wxID_HELP, "Help function"),
    wxToolBar:insertSeparator(ToolBar, 6),
    
    wxToolBar:addTool(ToolBar, ?wxID_ABOUT, "About", Info, 
		      [{shortHelp, "Info"}]),
    ...
    DL_Panel = wxPanel:new(MainPanel, [{winid, ?wxID_ANY}, 
				       {size, {100, 175}}, 
    ...
    %%Filename Text
    Text = wxStaticText:new(DL_Panel, 1, "Filename", 
			    [{style, ?wxALIGN_LEFT bor ?wxST_NO_AUTORESIZE}]),

    %%Time Text
    TimeText  = wxStaticText:new(DL_Panel, 3, "Time ", 
			     [{style, ?wxALIGN_LEFT bor ?wxST_NO_AUTORESIZE}]),
    %Folder ICON
    ...
    %%Gauge
    Range=100,
    NormalGauge = wxGauge:new(DL_Panel, 1, Range, [{pos, {167, 45}}, 
						   {size, {200, -1}},
    ...
    %%Calculation Text  
    CalcText  = wxStaticText:new(DL_Panel, 2, "Complete ", 
			     [{style, ?wxALIGN_LEFT bor ?wxST_NO_AUTORESIZE}]),
    
    %%URL Text   
    TrackText = wxStaticText:new(DL_Panel, 3, " - URL ", 
			      [{style, ?wxALIGN_LEFT bor ?wxST_NO_AUTORESIZE}]),
    ...
    wxSizer:add(FlexGridSizer, 25, 15, []),   
    wxSizer:add(FlexGridSizer, 25, 15, []),
    wxSizer:add(FlexGridSizer, StaticBitmap_Dl, [{proportion, 0}, 
						 {flag, ?wxALIGN_CENTER}]),  
    wxSizer:add(FlexGridSizer, Text, [{proportion, 0}, {flag, ?wxEXPAND}]),
    wxSizer:add(FlexGridSizer, TimeText, [{proportion, 0}, 
					  {flag, ?wxALIGN_CENTER}]),
    wxSizer:add(FlexGridSizer, StaticBitmap_Folder , [{proportion, 0},
						      {flag, ?wxALIGN_CENTER}]),
    wxSizer:add(FlexGridSizer, NormalGauge, [{proportion, 0}, 
					     {flag, ?wxEXPAND}]),
    wxSizer:add(FlexGridSizer, But101, [{proportion, 0}, 
					{flag, ?wxALIGN_CENTER}]),
    wxSizer:add(FlexGridSizer, 50, 30, []),
    wxSizer:add(FlexGridSizer, StatGridSizer, [{proportion, 0}, 
					       {flag, ?wxEXPAND}]),
    wxSizer:add(FlexGridSizer, 50, 30, []),
    ...
    wxSizer:add(OuterSizer, Advance_sizer,[{proportion, 0}, 
					   {flag, ?wxEXPAND}]),  
...
loop(State, Download_pid) ->
		...
		?wxID_OK ->
			...
			_ -> 
			    ...
			    TorrName  = Fname1 ++ Fname2,
                            ...
                            Create_string = "Created on: " ++ 
				create_date_and_time(),                       
			    wxWindow:refresh(Frame),
                            wxStaticText:setLabel(CreateText1104, 
						  Create_string), 
			    wxWindow:refresh(Frame),
			    wxStatusBar:setStatusText(StatusBar, 
						      "Download Started...", 
						      [{number, 0}]), 
			    wxStatusBar:setStatusText(StatusBar, TorrName, 
						      [{number, 1}]),
	...
	#wx{id= ?ADVANCE_FUNCTIONS_START, 
	    ...
	    wxToolBar:insertTool(ToolBar, 3, ?ADVANCE_FUNCTIONS_END, 
	...
	#wx{id= ?ADVANCE_FUNCTIONS_END, 
	    event=#wxCommand{type=command_menu_selected}} ->	 
	    
	    wxToolBar:removeTool(ToolBar, ?ADVANCE_FUNCTIONS_END), 
	    wxToolBar:insertTool(ToolBar, 3, ?ADVANCE_FUNCTIONS_START, 
				 "Advance", Advance_Start, [{shortHelp, 
							     "Advance"}, 
							    {longHelp, 
						 "Activate Advance Function"}]),
	...
	{percentage, {Percent, Dl_pieces}} ->
		...
		100 ->
		    ...
		    Complete_dialog =  wxMessageDialog:new(Frame, 
							"Succesfull Download!",
							   [{caption, 
							    "Congratulations"},
	...
	{'EXIT', _Pid, _Reason} ->
	    wxStatusBar:setStatusText(StatusBar, 
				      "Cannot connect to Torrent Tracker", 
				      [{number, 2}]),
	    Error2 =  wxMessageDialog:new(Frame, 
					  "Cannot connect to Torrent Tracker", 
...
create_date_and_time() ->     
    {H,M,S} = erlang:time(),
    {Y,Mo,D} = erlang:date(),
    DText = integer_to_list(Y) ++ "-" ++ integer_to_list(Mo) ++ "-" 
	++ integer_to_list(D),
    FText = integer_to_list(H) ++ ":" ++ integer_to_list(M) ++ ":" 
	++ integer_to_list(S),
